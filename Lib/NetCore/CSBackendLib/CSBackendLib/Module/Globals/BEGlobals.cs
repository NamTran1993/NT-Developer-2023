...
namespace BEBackendLib.Module.Globals
{
    public class BEGlobals
    {
        ...
        public static void Log(string fileName, string content, BELogger typeLogger = BELogger.NONE)
        {
            try
            {
                            ...
                            if (length > _MAX_SIZE_LOGFILE)
                            {
                                try
                                {
                                    ...
                                    if (arrFileName is not null && arrFileName.Length > 0)
                                    {
                                        ...
                                        file_info.BEFileRename(newFile);
                                    }
                                }
                                catch
                                {
        ...
        public static string CreateGUID(BEGuid type = BEGuid.DEFAULT, bool isUpper = false)
        {
            ...
            try
            {
                ...
                if (arrGuid is not null && arrGuid.Length > 0)
                {
                    res = type switch
                    {
                        BEGuid.DATE => string.Format("{0}-{1}-{2}", DateTime.Now.ToString("yyyy-MM-dd"), arrGuid[0], arrGuid[1]),
                        BEGuid.TIME => string.Format("{0}-{1}-{2}", DateTime.Now.ToString("hh-mm-ss"), arrGuid[0], arrGuid[1]),
                        BEGuid.DEFAULT_2 => string.Format("{0}-{1}", arrGuid[0], arrGuid[2]),
                        BEGuid.DEFAULT_3 => string.Format("{0}-{1}", arrGuid[0], arrGuid[3]),
                        BEGuid.DEFAULT_4 => string.Format("{0}-{1}", arrGuid[0], arrGuid[4]),
                        BEGuid.REMOVE_LINE => guid.Replace("-", ""),
                        _ => guid
                    };
